-- ACTIVAR SISTEMA AUTOMÁTICO COMPLETO - VERSION ARREGLADA
...
-- Primero insertar usuarios que faltan
DO $$
DECLARE
  user_record RECORD;
BEGIN
  FOR user_record IN 
    ...
    FROM auth.users
    WHERE id NOT IN (SELECT id FROM public.usuarios WHERE id IS NOT NULL)
  LOOP
    BEGIN
      INSERT INTO public.usuarios (id, email, nombre, telefono, created_at, updated_at)
      VALUES (
        user_record.id,
        user_record.email,
        COALESCE(user_record.raw_user_meta_data->>'nombre', 'Usuario'),
        COALESCE(user_record.raw_user_meta_data->>'telefono', NULL),
...
-- Luego insertar gimnasios que faltan
DO $$
DECLARE
  user_record RECORD;
BEGIN
  FOR user_record IN 
    SELECT id, email, raw_user_meta_data, created_at
    FROM auth.users
    WHERE id NOT IN (SELECT usuario_id FROM public.gimnasios WHERE usuario_id IS NOT NULL)
  LOOP
    BEGIN
      ...
      VALUES (
        gen_random_uuid(),
        user_record.id,
        COALESCE(user_record.raw_user_meta_data->>'nombreGimnasio', 'Mi Gimnasio'),
        COALESCE(user_record.raw_user_meta_data->>'direccion', 'Dirección pendiente'),
        COALESCE(user_record.raw_user_meta_data->>'telefono', NULL),
        user_record.email,
        '06:00',
        '22:00',
...
-- Verificar que cada usuario tiene su gimnasio
SELECT 
    'USUARIOS SIN GIMNASIO:' as info,
    COUNT(*) as total
FROM auth.users au
